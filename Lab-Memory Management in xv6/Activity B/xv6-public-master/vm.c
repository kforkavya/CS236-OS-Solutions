...
int present_or_not(struct proc* currproc, uint va)
{
  pte_t *pte;
...
void make_mmap_pte_with_present_bit_zero_and_set_lazy_bit_one(struct proc* currproc, uint va)
{
  pte_t *pte;
  pte = walkpgdir(currproc->pgdir, (void *)va, 1);
  *pte = *pte & ~PTE_P; //Remove Present Bit!
  *pte = *pte | PTE_LAZY; //Set Lazy Bit!
  switchuvm(currproc);
}

int check_if_lazy_or_not(struct proc* currproc, uint addr)
{
  pte_t *pte;
...
void remove_lazy_bit(struct proc* currproc, uint addr)
{
